...
// Index names by position (same order as TOKENS)
const INDEX_NAMES = [
    'NIFTY 50',
    'NIFTY BANK',
    'NIFTY MIDCAP 50',
    ...
    try {
        ...
        if (action === 'fetch') {
            ...
            if (result.status && result.data?.fetched) {
                ...
                result.data.fetched.forEach((item, index) => {
                    if (item) {
                        const ltp = parseFloat(item.ltp);
                        const close = parseFloat(item.close);
                        
                        // Use position-based name (GUARANTEED to match)
                        const name = INDEX_NAMES[index];
