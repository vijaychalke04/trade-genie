...
// Token to Name mapping (fallback if API doesn't return tradingsymbol)
const TOKEN_NAMES = {
    '99926000': 'NIFTY 50',
    '99926009': 'NIFTY BANK',
    '99926037': 'NIFTY MIDCAP 50',
            ...
            if (result.status && result.data?.fetched) {
                ...
                result.data.fetched.forEach(item => {
                    if (item) {
                        ...
                        // Use tradingsymbol from API, fallback to our mapping
                        const name = item.tradingsymbol || TOKEN_NAMES[token] || 'N/A';
                        
                        indices.push({
                            symbol: item.tradingsymbol || token,
                            name: name,
                            token: token,
                            ltp,
